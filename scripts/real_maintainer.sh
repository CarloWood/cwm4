...
if test "$(echo $GIT_COMMITTER_EMAIL | md5sum | cut -d \  -f 1)" = "$1"; then
  ...
  git submodule foreach 'if test -f "configure.m4" -a -z "$(git config -f $toplevel/.gitmodules submodule.$name.branch)"; then echo "$prefix $red""Setting submodule.$name.branch to master!$reset"; git -f $toplevel/.gitmodules submodule.$name.branch master; fi'
